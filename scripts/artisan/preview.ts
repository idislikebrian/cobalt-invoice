...
async function main(): Promise<void> {
  ...
  const entries = await fetchDetailedEntries({
    token,
    workspaceId,
    from: addDays(args.from, -1),
    through: addDays(args.through, 1),
  })
  const result = buildWeeklyPreviews(entries, weeks, sampleInvoice.sender)
  ...
  try {
    for (const preview of result.previews) {
      ...
      if (preview.audit.blockers.length > 0) continue
